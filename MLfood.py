...
if os.getuid() != 0:
    print(color.LIGHT_BLUE_1 + 'Docker needs to start with sudo mode' + color.GRAY)

# Check if there is the --reset-logs option to erase all the logs.
if "--reset-logs" in argv:
    print(color.ORANGE + "Are-you sure you want to delete all the saved logs? (y/n)")
    reset = input()
    reset.lower()
    if reset == "y":
...
try:
    nb = int(argv[1])
    if nb >= 50:
        print(color.ORANGE + "Are-you sure you want to start " + str(nb) + " compilation? (y/n)")
...
if nb <= 0:
    print(color.RED + "Please enter a non-zero positive integer." + color.GRAY)
    exit(0)
...
for i in range(nb):
    ...
    print(color.LIGHT_BLUE_1 + "Fetch logs and .config file to the folder ./Logs/" + logsFolder + " " + color.GRAY)
    os.system(stdlogs)
    os.system(errlogs)
    ...
    if "--no-clean" not in argv:
        print(color.LIGHT_PURPLE + "Cleaning containers . . .")
        os.system("sudo docker rm -v $(sudo docker ps -aq)")
        print("Clean done!")
        print("")
    elif argv[2] != null:
        print(color.RED + "Option " + argv[2] + " unknown." + color.GRAY)
        print("")
        exit(0)

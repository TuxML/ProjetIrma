...
def mlfood():
    ...
    if os.geteuid() != 0:
        ...
        if args.nbcompil >= 5:
            print(
                ORANGE + "You should run MLfood.py with 'sudo' to run a big number of compilations.")
    ...
    for i in range(args.nbcompil):
        ...
        if not args.no_logs:
            ...
            stdlogs = 'sudo docker cp ' + dock + \
                ':/TuxML/linux-4.13.3/logs/std.log ./Logs/' + logsFolder
            errlogs = 'sudo docker cp ' + dock + \
                ':/TuxML/linux-4.13.3/logs/err.log ./Logs/' + logsFolder
            configFile = 'sudo docker cp ' + dock + ':/TuxML/linux-4.13.3/.config ./Logs/' + \
                logsFolder + '/' + logsFolder + '.config'
            ...
            if not args.silent:
                print("")
                print("Fetch logs and .config file from container:" +
                      dock + " to the folder ./Logs/" + logsFolder)
                ...
                if args.fetch_kernel:
                    ...
                    subprocess.call("sudo docker cp " + dock + ":/TuxML/linux-4.13.3/vmlinux ./Logs/" +
                                    logsFolder, shell=True, stderr=subprocess.DEVNULL)
                    subprocess.call("sudo docker cp " + dock + ":/TuxML/linux-4.13.3/arch/x86/boot/compressed/vmlinux ./Logs/" +
                                    logsFolder + "/compressed-vmlinux", shell=True, stderr=subprocess.DEVNULL)
                    subprocess.call("sudo docker cp " + dock + ":/TuxML/linux-4.13.3/arch/x86/boot/bzImage ./Logs/" +
                                    logsFolder, shell=True, stderr=subprocess.DEVNULL)
                    for ext in extension:
                        subprocess.call(
                            "sudo docker cp " + dock + ":/TuxML/linux-4.13.3/arch/x86/boot/compressed/vmlinux.bin" + ext + " ./Logs/" + logsFolder, shell=True)
            ...
            # Silent mode enable
                ...
                if args.fetch_kernel:
                    ...
                    subprocess.call("sudo docker cp " + dock + ":/TuxML/linux-4.13.3/arch/x86/boot/bzImage ./Logs/" +
                                    logsFolder, shell=True, stderr=subprocess.DEVNULL)
                    for ext in extension:
                        subprocess.call("sudo docker cp " + dock + ":/TuxML/linux-4.13.3/arch/x86/boot/compressed/vmlinux.bin" +
                                        ext + " ./Logs/" + logsFolder, shell=True, stderr=subprocess.DEVNULL)
        ...
        if not args.no_clean:
            if not args.silent:
                ...
                subprocess.call(
                    "sudo docker stop $(sudo docker ps -aq)", shell=True)
                subprocess.call(
                    "sudo docker rm -v $(sudo docker ps -aq)", shell=True)
                print("")
                print("Clean done!")
                print("")
            else:
        ...
        if args.silent:
            print(LIGHT_BLUE_1 + "Docker #" + str(i) +
                  " done              ### SILENT MODE ###" + GRAY)
        print(LIGHT_BLUE_1 + "===============================================\n" + GRAY)

    #################### Section 12 ####################
    # The end
    print(LIGHT_BLUE_1 + "Your tamago... database Irma_DB ate " + GREEN + str(args.nbcompil * (args.incremental + 1)
                                                                              ) + LIGHT_BLUE_1 + " compilations data, come back later to feed it!" + GRAY, flush=True)
    print("", flush=True)
    print(LIGHT_BLUE_1 + "Total number of containers used: " +
          GREEN + str(args.nbcompil) + GRAY, flush=True)
    print(LIGHT_BLUE_1 + "Number of compilations in a container: " + GREEN + str(args.incremental + 1) + LIGHT_BLUE_1 +
          " ( 1 basic compilation + " + GREEN + str(args.incremental) + LIGHT_BLUE_1 + " incremental compilations )", flush=True)
    print(LIGHT_BLUE_1 + "Total number of compilations: " + GREEN +
          str(args.nbcompil * (args.incremental + 1)) + GRAY, flush=True)
    print("", flush=True)
...
def check_log():
        ...
        for folder in list:
            ...
            if os.path.isdir(path):
                ...
                if len(l) > 0:
                    ...
                    files = [f for f in l if os.path.isfile(
                        os.path.join(path, f))]
                    for file in files:
                        cmd = "wc -c " + path + "/" + file
                        raw_size = subprocess.Popen(
            ...
            # Case were there are files in the Logs/ level.
                ...
                raw_size = subprocess.Popen(
                    "wc -c " + path, shell=True, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
                tmp = raw_size.stdout.read().decode()
                tmp.replace("\n", " ")
                size = size + int(tmp.split()[-2])
    ...
    print(LIGHT_BLUE_1 + "You have " + color + printing + LIGHT_BLUE_1 + " " +
          unit + " of logs files, do not forget to clean up your logs." + GRAY)

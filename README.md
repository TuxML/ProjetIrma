...
The goal of TuxML ([Tux is the mascotte of the Linux Kernel](https://en.wikipedia.org/wiki/Tux_(mascot))) while ML stands for statistical machine learning) is to predict properties of Linux Kernel configurations (e.g., does the kernel compile? what's its size? does it boot?). 
...
We are developping tools, mainly based on Docker and Python, to massively compile and gather data about thousand of configuration kernels. 

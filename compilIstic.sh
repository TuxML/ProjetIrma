...
do
  ...
  do
    ...
    (ssh -o StrictHostKeyChecking=no -tt 14008349@$elem$m.istic.univ-rennes1.fr "echo \"$elem$m -- BEGIN\";~/TP/ProjetIrma/MLfood.py 1 --force-compilation-limits --dev --no-kernel --no-logs --no-check-log --silent > /dev/null; echo \"$elem$m -- DONE\"; exit")&
  done
done
